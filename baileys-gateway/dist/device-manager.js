import { default as makeWASocket, useMultiFileAuthState, Browsers, DisconnectReason, fetchLatestBaileysVersion, } from "@whiskeysockets/baileys";
...
const BACKEND_EVENT_URL = process.env.BACKEND_EVENT_URL || "http://localhost:8080/internal/events/baileys";
...
const AUTO_RESET_AUTH_ON_LOGGED_OUT = process.env.AUTO_RESET_AUTH_ON_LOGGED_OUT !== "false";
// Store active device sessions in memory
const activeSessions = new Map();
const reconnectAttempts = new Map();
const reconnectTimers = new Map();
function clearReconnectTimer(deviceKey) {
...
function resetReconnectState(deviceKey) {
    clearReconnectTimer(deviceKey);
    reconnectAttempts.delete(deviceKey);
}
function removeSessionFiles(deviceKey) {
...
export async function startDevice(deviceKey) {
        ...
        socket.ev.on("connection.update", async (update) => {
            ...
            if (connection === "close") {
                ...
                log.warn({
                    deviceKey,
                    statusCode,
                    disconnectReason: DisconnectReason[statusCode] || "unknown",
...
export async function stopDevice(deviceKey) {
...
export async function sendMessage(deviceKey, toNumber, messageBody) {
...
function normalizeRecipientJid(toNumber) {
...
async function publishEvent(type, payload) {
    try {
        await axios.post(BACKEND_EVENT_URL, { type, payload }, {
            timeout: 5000,
            headers: {
